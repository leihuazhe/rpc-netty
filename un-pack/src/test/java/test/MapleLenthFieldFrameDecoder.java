...
/**
 * 一种解码器，通过消息中的length字段的值 动态地分割接收到的{@link ByteBuf}
 * 当您解码一个二进制消息时,而且该消息具有一个整数头字段,表示消息正文或整个消息的长度时,这个类特别有用。
 * <p>
 *
 * {@link io.netty.handler.codec.LengthFieldBasedFrameDecoder} 有很多配置参数,
 * 所以它能解码任何的包含长度字段值的消息,在专有的client-server 协议中经常看到.
 *
 *  Here are some example that will give you the basic idea on which option does what.
 *
 * <h3>2 bytes length field at offset 0, do not strip header</h3>
 * <p>
 * The value of the length field in this example is <tt>12 (0x0C)</tt> which
 * represents the length of "HELLO, WORLD".
 * 默认情况下，解码器假设 length字段表示length字段后面的字节数。
 ...
 */
public class MapleLenthFieldFrameDecoder extends ByteToMessageDecoder {
    ...
    /**
     * 跳过的字节数，根据需要我们跳过 lengthFieldLength 个字节，以便接收端直接接受到不含“长度属性”的内容
     */
     ...
     */
    public MapleLenthFieldFrameDecoder(
            int maxFrameLength,
